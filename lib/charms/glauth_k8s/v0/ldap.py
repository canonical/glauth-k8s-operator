...
from pydantic import ConfigDict, StrictBool, ValidationError, version
...
if PYDANTIC_IS_V1:
    ...
    def Field(*args: Any, **kwargs: Any) -> FieldV1:  # noqa N802
        if "frozen" in kwargs:
            kwargs["allow_mutations"] = kwargs["frozen"]
    ...
    class ModelCompatibilityMeta(ModelMetaclass):
        def __init__(self, name: str, bases: Tuple[object], dct: Dict) -> None:
            method_list = [
                f for f in self.__dict__.values() if callable(f) and f in encoders_config
            ]
            ...
            for f in method_list:
                field_name = encoders_config[f]
                self._encoders[field_name] = f
